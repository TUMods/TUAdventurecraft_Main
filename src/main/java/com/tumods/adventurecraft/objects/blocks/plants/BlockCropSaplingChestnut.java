...
public class BlockCropSaplingChestnut extends BlockCrops implements IGrowable {
	public static final PropertyInteger CHESTNUT_SAPLING_AGE = PropertyInteger.create("age", 0, 3);
	
	private static final AxisAlignedBB[] CHESTNUT_SAPLING_AABB = new AxisAlignedBB[] {
...
//			new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D)    // Stage 8
	...
	public BlockCropSaplingChestnut(String name) {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (!worldIn.isRemote && this.isMaxAge(state))
			worldIn.setBlockState(pos, BlockInit.SAPLING_CHESTNUT.getDefaultState());
	}
	...
	public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
		if (state.getValue(getAgeProperty()) == 3)
			worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), 
				new ItemStack(BlockInit.SAPLING_CHESTNUT, 1)));
		else
			worldIn.spawnEntity(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), 
					new ItemStack(ItemInit.PRODUCE_CHESTNUT, 1)));
	}
	
	@Override
	protected PropertyInteger getAgeProperty() {
		return CHESTNUT_SAPLING_AGE;
	}
    ...
    {
        return new BlockStateContainer(this, new IProperty[] {CHESTNUT_SAPLING_AGE});
    }
	
	@Override
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
		return CHESTNUT_SAPLING_AABB[((Integer)state.getValue(getAgeProperty())).intValue()];
	}
}
